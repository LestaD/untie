...
function createShapeTypeChecker(shapeTypes) {
  function validate(props, propName, propFullName) {
    ...
    return Object.keys(error).length ? errors : null;
  }
  return createChainableTypeChecker(validate);
}
...
var Validator = (function() {
  ...
  Validator.validate = function(data) {
    ...
    for (var kid in keys) {
      ...
      var error = validator(data, propName, propName);
      if (error && (Object.keys(error).length || error instanceof Error)) {
...
function createValidator(schema) {
  ...
  Object.getOwnPropertyNames(Validator)
    .filter(function (prop) {
      return typeof Validator[prop] === 'function';
    })
    .forEach(function (method) {
      newValidator[method] = Validator[method].bind(newValidator);
    });
